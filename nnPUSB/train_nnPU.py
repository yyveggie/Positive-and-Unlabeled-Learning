...
def train_pu(x_train, t_train, x_test, t_test, pi, epoch, model, optimizer, batchsize=5000, xp=np):
    ...
    for ep in six.moves.range(1, epoch + 1):
        ...
        for i in six.moves.range(0, N, batchsize):
            ...
            count += 1
        loss_step /= count
        acc1, f1_binary1 = test(x_test, t_test, model, quant=False, xp=xp, batchsize=batchsize)
        acc2, f1_binary2 = test(x_test, t_test, model, quant=True, pi=pi, xp=xp, batchsize=batchsize)
        acc1_list.append(acc1)
        acc2_list.append(acc2)
        f1_binary_list1.append(f1_binary1)
        f1_binary_list2.append(f1_binary2)
    ...
    f1_binary_list1 = np.array(f1_binary_list1)
    f1_binary_list2 = np.array(f1_binary_list2)

    return model, optimizer, acc1_list, acc2_list, f1_binary_list1, f1_binary_list2
