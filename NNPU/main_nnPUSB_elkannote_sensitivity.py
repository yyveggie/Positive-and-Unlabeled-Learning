...
def experiment():
    ...
    est_pre_acc_pu0 = np.zeros((ite, epoch))
    est_pre_acc_pu1 = np.zeros((ite, epoch))
    est_pre_acc_pu2 = np.zeros((ite, epoch))
    est_pre_acc_pu3 = np.zeros((ite, epoch))
    ...
    for i in range(ite):
        ...
        model, optimizer, loss_list, acc, acc0, pre, rec, pre0, rec0 = train_pu(x_train, t_train, x_test, t_test,
                                                                                pi - 0.05, epoch, model, optimizer,
                                                                                batchsize, xp)
        model, optimizer, loss_list, acc, acc1, pre, rec, pre1, rec1 = train_pu(x_train, t_train, x_test, t_test,
                                                                                pi + 0.05, epoch, model, optimizer,
                                                                                batchsize, xp)
        model, optimizer, loss_list, acc, acc2, pre, rec, pre2, rec2 = train_pu(x_train, t_train, x_test, t_test,
                                                                                pi + 0.1, epoch, model, optimizer,
                                                                                batchsize, xp)
        model, optimizer, loss_list, acc, acc3, pre, rec, pre3, rec3 = train_pu(x_train, t_train, x_test, t_test,
                                                                                pi + 0.2, epoch, model, optimizer,
                                                                                batchsize, xp)
